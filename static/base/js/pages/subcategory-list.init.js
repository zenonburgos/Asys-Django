...
var perPage = 8,
    options = {
        valueNames: ["id", "categoria_name", "subcategoria_name", "fc", "date", "phone", "status"],
        page: perPage,
        pagination: !0,
    ...
    subcategoryList = new List("subcategoryList", options).on("updated", function(e) {
...
xhttp.onload = function() {
    JSON.parse(this.responseText).forEach(e => {
        categoryList.add({
            ...
            categoria_name: e.categoria_name,
            subcategoria_name: e.subcategoria_name,
            fc: e.fc,
            date: e.date,
            phone: e.phone,
            status: isStatus(e.status)
        }), refreshCallbacks()
    }), subcategoryList.remove("id", '<a href="javascript:void(0);" class="fw-medium link-primary">#VZ2101</a>')
}, xhttp.open("GET", "{% static 'base/json/subcategoria-list.json' %}"), xhttp.send(), isCount = (new DOMParser).parseFromString(subcategoryList.items.slice(-1)[0]._values.id, "text/html");
var isValue = isCount.body.firstElementChild.innerHTML,
    idField = document.getElementById("id-field"),
    categoriaNameField = document.getElementById("categorianame-field"),
    subcategoriaNameField = document.getElementById("subcategorianame-field"),
...
function filterContact(e) {
    var t = e;
    subcategoryList.filter(function(e) {
...
refreshCallbacks(), document.getElementById("showModal").addEventListener("show.bs.modal", function(e) {
    e.relatedTarget.classList.contains("edit-item-btn") ? (document.getElementById("exampleModalLabel").innerHTML = "Edit subcategoria", document.getElementById("showModal").querySelector(".modal-footer").style.display = "block", document.getElementById("add-btn").style.display = "none", document.getElementById("edit-btn").style.display = "block") : e.relatedTarget.classList.contains("add-btn") ? (document.getElementById("exampleModalLabel").innerHTML = "Add subcategoria", document.getElementById("showModal").querySelector(".modal-footer").style.display = "block", document.getElementById("edit-btn").style.display = "none", document.getElementById("add-btn").style.display = "block") : (document.getElementById("exampleModalLabel").innerHTML = "List subcategoria", document.getElementById("showModal").querySelector(".modal-footer").style.display = "none")
}), ischeckboxcheck(), document.getElementById("showModal").addEventListener("hidden.bs.modal", function() {
    clearFields()
}), document.querySelector("#subcategoryList").addEventListener("click", function() {
    refreshCallbacks(), ischeckboxcheck()
});
var table = document.getElementById("subcategoriaTable"),
...
addBtn.addEventListener("click", function(e) {
    "" !== categoriaNameField.value && "" !== subcategoriaNameField.value && "" !== fcField.value && "" !== dateField.value && "" !== phoneField.value && (subcategoryList.add({
        id: '<a href="javascript:void(0);" class="fw-medium link-primary">#VZ' + count + "</a>",
        categoria_name: categoriaNameField.value,
        subcategoria_name: subcategoriaNameField.value,
        fc: fcField.value,
    ...
    document.getElementById("exampleModalLabel").innerHTML = "Edit subcategoria", subcategoryList.get({
        id: idField.value
    }).forEach(function(e) {
...
function refreshCallbacks() {
    removeBtns.forEach(function(e) {
        e.addEventListener("click", function(e) {
            e.target.closest("tr").children[1].innerText, itemId = e.target.closest("tr").children[1].innerText, subcategoryList.get({
                id: itemId
            }).forEach(function(e) {
                ...
                deleteid.body.firstElementChild.innerHTML == itemId && document.getElementById("delete-record").addEventListener("click", function() {
                    subcategoryList.remove("id", t.outerHTML), document.getElementById("deleteRecordModal").click()
                })
            })
        })
        ...
        e.addEventListener("click", function(e) {
            e.target.closest("tr").children[1].innerText, itemId = e.target.closest("tr").children[1].innerText, subcategoryList.get({
                ...
                t == itemId && (idField.value = t, categoriaNameField.value = e._values.categoria_name, subcategoriaNameField.value = e._values.subcategoria_name, fcField.value = e._values.fc, dateField.value = e._values.date, phoneField.value = e._values.phone, statusVal && statusVal.destroy(), statusVal = new Choices(statusField, {
                    searchEnabled: !1
...
function clearFields() {
    categoriaNameField.value = "", subcategoriaNameField.value = "", fcField.value = "", dateField.value = "", phoneField.value = ""
}
...
function deleteMultiple() {
    ...
    if ("undefined" != typeof ids_array && 0 < ids_array.length) {
        ...
        ids_array.forEach(function(e) {
            subcategoryList.remove("id", `<a href="javascript:void(0);" class="fw-medium link-primary">${e}</a>`)

...
var perPage = 8,
    options = {
        valueNames: ["id", "um_name", "fc", "date", "phone", "status"],
        page: perPage,
        pagination: !0,
    ...
    umList = new List("umList", options).on("updated", function(e) {
...
xhttp.onload = function() {
    ...
    }), umList.remove("id", '<a href="javascript:void(0);" class="fw-medium link-primary">#VZ2101</a>')
}, xhttp.open("GET", "{% static 'base/json/um-list.json' %}"), xhttp.send(), isCount = (new DOMParser).parseFromString(umList.items.slice(-1)[0]._values.id, "text/html");
var isValue = isCount.body.firstElementChild.innerHTML,
    idField = document.getElementById("id-field"),
    umNameField = document.getElementById("umname-field"),
...
refreshCallbacks(), document.getElementById("showModal").addEventListener("show.bs.modal", function(e) {
    e.relatedTarget.classList.contains("edit-item-btn") ? (document.getElementById("exampleModalLabel").innerHTML = "Edit um", document.getElementById("showModal").querySelector(".modal-footer").style.display = "block", document.getElementById("add-btn").style.display = "none", document.getElementById("edit-btn").style.display = "block") : e.relatedTarget.classList.contains("add-btn") ? (document.getElementById("exampleModalLabel").innerHTML = "Add um", document.getElementById("showModal").querySelector(".modal-footer").style.display = "block", document.getElementById("edit-btn").style.display = "none", document.getElementById("add-btn").style.display = "block") : (document.getElementById("exampleModalLabel").innerHTML = "List um", document.getElementById("showModal").querySelector(".modal-footer").style.display = "none")
}), ischeckboxcheck(), document.getElementById("showModal").addEventListener("hidden.bs.modal", function() {
    clearFields()
}), document.querySelector("#umList").addEventListener("click", function() {
...
addBtn.addEventListener("click", function(e) {
    "" !== umNameField.value && "" !== fcField.value && "" !== dateField.value && "" !== phoneField.value && (umList.add({
        id: '<a href="javascript:void(0);" class="fw-medium link-primary">#VZ' + count + "</a>",
        um_name: umNameField.value,
        fc: fcField.value,
    ...
    document.getElementById("exampleModalLabel").innerHTML = "Edit um", umList.get({
        id: idField.value
    }).forEach(function(e) {
...
function refreshCallbacks() {
    removeBtns.forEach(function(e) {
        e.addEventListener("click", function(e) {
            e.target.closest("tr").children[1].innerText, itemId = e.target.closest("tr").children[1].innerText, umList.get({
                id: itemId
            }).forEach(function(e) {
                ...
                deleteid.body.firstElementChild.innerHTML == itemId && document.getElementById("delete-record").addEventListener("click", function() {
                    umList.remove("id", t.outerHTML), document.getElementById("deleteRecordModal").click()
                })
            })
        })
        ...
        e.addEventListener("click", function(e) {
            e.target.closest("tr").children[1].innerText, itemId = e.target.closest("tr").children[1].innerText, umList.get({
                ...
                t == itemId && (idField.value = t, umNameField.value = e._values.um_name, fcField.value = e._values.fc, dateField.value = e._values.date, phoneField.value = e._values.phone, statusVal && statusVal.destroy(), statusVal = new Choices(statusField, {
                    searchEnabled: !1
...
function clearFields() {
    umNameField.value = "", fcField.value = "", dateField.value = "", phoneField.value = ""
}
...
function deleteMultiple() {
    ...
    if ("undefined" != typeof ids_array && 0 < ids_array.length) {
        ...
        ids_array.forEach(function(e) {
            umList.remove("id", `<a href="javascript:void(0);" class="fw-medium link-primary">${e}</a>`)

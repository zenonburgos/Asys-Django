...
var perPage = 8,
    options = {
        valueNames: ["id", "categoria_name", "fc", "date", "phone", "status"],
        page: perPage,
        pagination: !0,
    ...
    categoryList = new List("categoryList", options).on("updated", function(e) {
...
xhttp.onload = function() {
    ...
    }), categoryList.remove("id", '<a href="javascript:void(0);" class="fw-medium link-primary">#VZ2101</a>')
}, xhttp.open("GET", "{% static 'base/json/categoria-list.json' %}"), xhttp.send(), isCount = (new DOMParser).parseFromString(categoryList.items.slice(-1)[0]._values.id, "text/html");
...
function filterContact(e) {
    var t = e;
    categoryList.filter(function(e) {
...
refreshCallbacks(), document.getElementById("showModal").addEventListener("show.bs.modal", function(e) {
    e.relatedTarget.classList.contains("edit-item-btn") ? (document.getElementById("exampleModalLabel").innerHTML = "Edit categoria", document.getElementById("showModal").querySelector(".modal-footer").style.display = "block", document.getElementById("add-btn").style.display = "none", document.getElementById("edit-btn").style.display = "block") : e.relatedTarget.classList.contains("add-btn") ? (document.getElementById("exampleModalLabel").innerHTML = "Add categoria", document.getElementById("showModal").querySelector(".modal-footer").style.display = "block", document.getElementById("edit-btn").style.display = "none", document.getElementById("add-btn").style.display = "block") : (document.getElementById("exampleModalLabel").innerHTML = "List categoria", document.getElementById("showModal").querySelector(".modal-footer").style.display = "none")
...
var table = document.getElementById("categoriaTable"),
...
addBtn.addEventListener("click", function(e) {
    "" !== categoriaNameField.value && "" !== fcField.value && "" !== dateField.value && "" !== phoneField.value && (categoryList.add({
    ...
    document.getElementById("exampleModalLabel").innerHTML = "Edit categoria", categoryList.get({
        id: idField.value
    }).forEach(function(e) {
...
function refreshCallbacks() {
    removeBtns.forEach(function(e) {
        e.addEventListener("click", function(e) {
            e.target.closest("tr").children[1].innerText, itemId = e.target.closest("tr").children[1].innerText, categoryList.get({
                id: itemId
            }).forEach(function(e) {
                ...
                deleteid.body.firstElementChild.innerHTML == itemId && document.getElementById("delete-record").addEventListener("click", function() {
                    categoryList.remove("id", t.outerHTML), document.getElementById("deleteRecordModal").click()
                })
            })
        })
        ...
        e.addEventListener("click", function(e) {
            e.target.closest("tr").children[1].innerText, itemId = e.target.closest("tr").children[1].innerText, categoryList.get({
                ...
                t == itemId && (idField.value = t, categoriaNameField.value = e._values.categoria_name, fcField.value = e._values.fc, dateField.value = e._values.date, phoneField.value = e._values.phone, statusVal && statusVal.destroy(), statusVal = new Choices(statusField, {
                    searchEnabled: !1
...
function clearFields() {
    categoriaNameField.value = "", fcField.value = "", dateField.value = "", phoneField.value = ""
}
...
function deleteMultiple() {
    ...
    if ("undefined" != typeof ids_array && 0 < ids_array.length) {
        ...
        ids_array.forEach(function(e) {
            categoryList.remove("id", `<a href="javascript:void(0);" class="fw-medium link-primary">${e}</a>`)
